...
//			direction:			"ltr",
			...
			buttons: 			"bold,italic,underline,strike,sup,sub,|,img,video,link,|,bullist,numlist,|,fontcolor,fontsize,fontfamily,|,justifyleft,justifycenter,justifyright,|,quote,spoiler,usertag,code,table,removeFormat",
			allButtons: {
				bold : {
					title: CURLANG.bold,
				...
				quote : {
					...
					//subInsert: true,
					transform : { 
						'<div class="quotetext">{SELTEXT}</div>':"[quote]{SELTEXT}[/quote]",
						'<div class="quotetext"><a><b>{AUTHOR} said:</a></b><i class="dnone">{MSGID}</i><br>{SELTEXT}</div>':'[quote={AUTHOR} message={MSGID}]{SELTEXT}[/quote]'
					}
				},
				spoiler : {
				...
				usertag: {
					...
					removeCallback: function(wysibb,$root){
						var rng = wysibb.getRange();
						var shtml = wysibb.getSelectText();
						var rnode = wysibb.getSelectNode();
						
						if(shtml){
							wysibb.setCursorByEl($root);
							$root.replaceWith(shtml)
						}
						else if(rng.endOffset === rnode.length){
							$root.append(' ');
							wysibb.setCursorByEl($root);
						}
						else{
							wysibb.setCursorByEl($root);
							$root.replaceWith(rnode);
						}
					}
				},
				// nextBB : {
				// 	title: CURLANG.nextBB,
				...
				video: {
					...
					transform: {
						'<iframe src="https://www.youtube.com/embed/{SRC}" width="340" height="200" frameborder="0"></iframe>':'[yt]{SRC}[/yt]'
					}
				},
	...
	$.wysibb.prototype = {
		...
		buildToolbar: function() {
			...
			this.$txtArea[0].$modeSwitch = $bbsw.children(".wysibb-toolbar-btn");
		...
		initHotkeys: function() {
			...
			var klist2 = {arrowdown:40, arrowup:38, arrowleft:37, arrowright:39};
					...
					var keys = opt.hotkey.split("+");
 					if (keys && keys.length>=2) {
						...
						if (metasum>0) {
							...
							let temp = klist.indexOf(key);
							let name = temp > -1 ? temp + 48 : klist2[key];
							this.hotkeys["m"+metasum]["k"+name]=cmd;
						}
					}
				}
			},this))
		...
		presskey: function(e) {
			if (e.ctrlKey==true || e.shiftKey==true || e.altKey==true) {
				...
				if (this.hotkeys["m"+metasum] && this.hotkeys["m"+metasum]["k"+e.which]) {
					this.execCommand(this.hotkeys["m"+metasum]["k"+e.which],[false,'hotkey']);
					e.preventDefault();
					return false;
				}
			}
		...
		execCommand: function(command,value) {
			...
			if(value && value.constructor.name === 'Array'){
				if(value[1] === 'hotkey' && opt.hotkeyCall) {return opt.hotkeyCall();}
				value = value[0];
			}
			...
			if (this.options.bbmode) {
				...
				if (opt.bbSelector) {
					for (var i=0; i<opt.bbSelector.length; i++) {
						var regx = (opt.regx ? opt.regx[opt.bbSelector[i].match(/.+?\]|@/)[0]] : null) || [];
						var b = this.isBBContain(opt.bbSelector[i],regx[0]);
						if (b) {
							return this.getParams(b,opt.bbSelector[i],b[1],regx[0]);
						}
					}
				}
				return false;
		...
		wbbRemoveCallback: function(command,clear) {
			...
			if (this.options.bbmode) {
				...
				$.each(opt.bbSelector,$.proxy(function(i,bbcode) {
					var regx = (opt.regx ? opt.regx[bbcode.match(/.+?\]|@/)[0]] : null) || [];
					var stext = regx[1] || bbcode.match(/\{[\s\S]+?\}/g);
					$.each(stext,function(n,s) {
						if (s.toLowerCase()=="{seltext}") {stextnum=n;return false}
					});
					var a = this.isBBContain(bbcode,regx[0]);
					if (a) {
				...
				$.each(opt.rootSelector,$.proxy(function(i,s) {
					...
					if(opt.removeCallback){
						opt.removeCallback(this,$root)
					}
					else if ($root.is("span[wbb]") || !$root.is("span,font")) { //remove only blocks
			...
			$.each(this.options.btnlist,$.proxy(function(i,b){
				if (b!="|" && b!="-") {
					...
					$.each(this.options.allButtons[b].transform,$.proxy(function(html,bb) {
						html = html.replace(/\n/g,""); //IE 7,8 FIX
						var a=[];
						let btnregx = this.options.allButtons[b].regx;
						btnregx ? btnregx = btnregx[bb.match(/.+?\]|@/)[0]] : null;
						if(btnregx){
							bb = btnregx[0];
							a = btnregx[1]
						} else {
		...
		isBBContain: function(bbcode,regx) {
			var pos=this.getCursorPosBB();
			var b = regx || this.prepareRGX(bbcode);
		...
		getParams: function(src,s,offset,regx) {
			var params={};
			if (this.options.bbmode) {
				//bbmode
				var stext = s.match(/\{[\s\S]+?\}/g);
				s = regx || this.prepareRGX(s);
